...
def image_set_distortion(image_set):
    temp = []
    # counter = 0
    for image in image_set:
        image = image.reshape(28, 28)
        image = elastic_transform(image, 36, 8)
        image = image.flatten()
        temp.append(image)
        # print(counter)
        # counter +=1
    return np.array(temp).reshape(50000, 784)
...
def load_data(dataset):
    ...
    # train_set_x_copy = train_set_x.copy()
    # print(np.shape(train_set_x_copy))
    # print(" ... data augmentation")
    # train_set_x_copy = image_set_distortion(train_set_x_copy)
    # train_set_y = np.concatenate((train_set_y, train_set_y), axis=0)
    # train_set_x = np.concatenate((train_set_x, train_set_x_copy), axis=0)
...
def records_file_maker(name, lst1, lst2, test_val):
    """ this write given list into a .csv file"""
    import csv
